...
Input:  Raw data files (prices, fundamentals, news, rankings, money flow)
Output: data/all_features_v2.parquet

Runtime: ~2-4 hours depending on data volume
Run frequency: Monthly or when new data arrives

Data path configuration:
  - Set RAW_DATA_ROOT below to your raw data directory
  - Expected structure:
    {RAW_DATA_ROOT}/prices/{YYYYMMDD}.parquet
    {RAW_DATA_ROOT}/other/{YYYYMMDD}.parquet
    {RAW_DATA_ROOT}/news/analysis_{YYYY-MM-DD}.json
    {RAW_DATA_ROOT}/rank/{YYYYMMDD}.parquet
    {RAW_DATA_ROOT}/moneyflow/{YYYYMMDD}.parquet
    {RAW_DATA_ROOT}/ths_news/{YYYYMMDD}.parquet
"""
import os
import sys
import pandas as pd
...
RAW_DATA_ROOT = r'D:\iquant_data\data_v2'
RAW_PRICE_DIR = os.path.join(RAW_DATA_ROOT, 'prices')
RAW_OTHER_DIR = os.path.join(RAW_DATA_ROOT, 'other')
RAW_NEWS_DIR = os.path.join(RAW_DATA_ROOT, 'news')
RAW_RANK_DIR = os.path.join(RAW_DATA_ROOT, 'rank')
RAW_MONEYFLOW_DIR = os.path.join(RAW_DATA_ROOT, 'moneyflow')
RAW_THS_NEWS_DIR = os.path.join(RAW_DATA_ROOT, 'ths_news')
...
def get_all_dates():
    if not os.path.exists(RAW_PRICE_DIR):
        print(f"ERROR: Price directory not found: {RAW_PRICE_DIR}")
        return []
    files = [f.replace('.parquet', '') for f in os.listdir(RAW_PRICE_DIR) if f.endswith('.parquet')]
    return sorted(files)
...
def build_listing_date_map():
    ...
    for d in tqdm(all_dates, desc="Building listing date map"):
...
def run():
    print("=" * 80)
    print("Step 1: Build Full Feature Dataset")
    print("=" * 80)

    if not os.path.exists(RAW_PRICE_DIR):
        print(f"ERROR: Raw data directory not found: {RAW_PRICE_DIR}")
        print("Please configure RAW_DATA_ROOT at the top of this script.")
        return

    first_seen, _ = build_listing_date_map()
    print(f"Listing date map: {len(first_seen)} stocks")
    ...
    for i in tqdm(range(len(available_dates) - 30), desc="Building features"):
        d_curr = available_dates[i]
        d_t2 = available_dates[i + 2]
        ...
        p_t2 = os.path.join(RAW_PRICE_DIR, f"{d_t2}.parquet")
        if os.path.exists(p_t2):
            try:
                df_t2 = pd.read_parquet(p_t2)
                t2_data = df_t2[['ts_code', 'open']].rename(columns={'open': 'entry_price'})
                features = pd.merge(features, t2_data, on='ts_code', how='left')
            except Exception:
                pass

        d_exit = available_dates[i + TARGET_HORIZON_DAYS] if i + TARGET_HORIZON_DAYS < len(available_dates) else None
        if d_exit:
            ...
            if os.path.exists(p_exit):
                try:
                    ...
                    if 'exit_price_1d' in features.columns and 'entry_price' in features.columns:
                        features['return_1d'] = features['exit_price_1d'] / features['entry_price'] - 1
                except Exception:
                    pass
    ...
    if training_data:
        ...
        print(f"Data: {len(result)} rows, {len(result.columns)} columns")
        return result
