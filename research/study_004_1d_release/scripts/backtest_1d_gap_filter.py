...
COMBOS = []
for th in [0.5, 0.52, 0.54, 0.56, 0.58, 0.6, 0.62, 0.64, 0.66]:
    for mp in [3, 5, 10]:
        for sl in [0.0, -0.03, -0.05, -0.07, -0.1]:
            for tp in [0.0, 0.05, 0.08, 0.1]:
                COMBOS.append({'threshold': th, 'max_pos': mp, 'stop_loss': sl, 'take_profit': tp})
...
def load_ohlc_and_pctchg():
    ...
    ohlc_lookup = {}
    pctchg_lookup = {}
    has_pct_chg = 'pct_chg' in feat.columns
    has_pre_close = 'pre_close' in feat.columns

    for row in feat.itertuples(index=False):
        key = (row.ts_code, row.trade_date)
        ohlc_lookup[key] = (row.open, row.high, row.low, row.close)
        
        if has_pct_chg and pd.notna(row.pct_chg):
            pctchg_lookup[key] = row.pct_chg
        elif has_pre_close and pd.notna(row.pre_close) and row.pre_close > 0:
...
def backtest_1d(pred_df, ohlc_lookup, pctchg_lookup, threshold, max_pos, stop_loss=0.0, take_profit=0.0):
    hold_days = 2
    ...
    for day_i, d in enumerate(trading_dates):
        ...
        for pos_i in open_idx[buy_mask]:
            ...
            ohlc_t0 = ohlc_lookup.get((ts_codes_arr[pos_i], trading_dates[entry_date_idx[pos_i]]))
            if ohlc_t0 is not None:
                prev_close = ohlc_t0[3]
                if prev_close > 0:
                    gap_up = (o - prev_close) / prev_close
                    if not (0.02 < gap_up < 0.06):
                        n_skip_gap += 1
                        status[pos_i] = 0
                        continue
        ...
        for j in range(len(active_positions)):
            ...
            if sl_price[pos_i] > 0 and o <= sl_price[pos_i]:
                if at_limit_down:
                    ...
                    last_price[pos_i] = o
                    triggered = True
            elif tp_price[pos_i] > 0 and o >= tp_price[pos_i]:
                # 无论是否涨停，由于是卖出，有人买就能卖。所以假设能卖出在开盘价。
                daily_pnl[day_i] += pos_size * (o - prev) / prev - pos_size * SELL_COST
                status[pos_i] = 0
                last_price[pos_i] = o
                triggered = True
                ...
                triggered = True
            elif tp_price[pos_i] > 0 and h >= tp_price[pos_i]:
...
def run():
    ...
    for ci, combo in enumerate(COMBOS):
        ...
        for period_name, start, end in PERIODS:
            ...
            all_results[key] = {**stats, 'label': label, 'period': period_name,
                                 'threshold': th, 'max_pos': mp, 'stop_loss': sl, 'take_profit': tp}
    ...
    best_per_th = {}
    for key, s in all_results.items():
        if s['period'] == 'test_2025_2026' and s['cagr'] > 0:
            th = s['threshold']
            if th not in best_per_th or s['sharpe'] > best_per_th[th]['sharpe']:
                best_per_th[th] = s

    for th in sorted(best_per_th.keys()):
        s = best_per_th[th]
        tp_str = f"tp={s['take_profit']:.0%}" if s['take_profit'] > 0 else "tp=+0%"
        print(f"  thresh={th}: pos={s['max_pos']}, sl={s['stop_loss']:.0%}, {tp_str}, "
              f"test_sharpe={s['sharpe']:.2f}, test_cagr={s['cagr']:.1%}, win_rate={s['win_rate_days']:.1%}", flush=True)
    print("==========================================================================================", flush=True)
