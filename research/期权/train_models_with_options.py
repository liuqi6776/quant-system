"""
train_models_with_options.py
基于 features_005_options.parquet，进行按月滚动的 Walk-Forward 训练。
包含全新的期权大盘特征作为模型的输入，使模型具备大盘择时与风险防御的全局感知能力。
"""
...
FEATURES_FILE = os.path.join(SCRIPT_DIR, '..', 'study_005_1d_advanced', 'data', 'features_005_options.parquet')
OUTPUT = os.path.join(SCRIPT_DIR, '..', 'study_005_1d_advanced', 'predictions', 'predictions_005_options_wf.parquet')
os.makedirs(os.path.dirname(OUTPUT), exist_ok=True)

TRAIN_START = '20200101'
HALF_LIFE_DAYS = 252.0  # 半衰期 1 年
...
def run():
    t0 = time.time()
    print("Loading features 005 with Options...", flush=True)
    if not os.path.exists(FEATURES_FILE):
        print(f"Error: Feature file not found at {FEATURES_FILE}", flush=True)
        return
    ...
    print(f"Rows: {len(df)}, Total Features: {len(feature_cols)}", flush=True)
    print("New Option Features detected:")
    ...
    if all_preds:
        ...
        print(f"\nSaved prediction with options to: {OUTPUT} ({len(combined)} rows)", flush=True)
